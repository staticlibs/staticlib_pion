...
public:    
     ...
     */
    pion::tribool finish_header_parsing(http::message& http_msg, asio::error_code& ec);
...
protected:
     ...
     */
    pion::tribool parse_chunks(http::message::chunk_cache_t& chunk_buffers, asio::error_code& ec);
     ...
     *                        indeterminate = message is not yet finished
     */
    pion::tribool consume_content(http::message& http_msg, asio::error_code& ec);
    ...
    return (name.empty() || name[0] == '$' || (set_cookie_header &&
        (
            ...
            // Set-Cookie header is a cookie-pair unless <name> is a known cookie attribute.
               pion::algorithm::iequals(name, "Comment")
            || pion::algorithm::iequals(name, "Domain")
            || pion::algorithm::iequals(name, "Max-Age")
            || pion::algorithm::iequals(name, "Path")
            || pion::algorithm::iequals(name, "Secure")
            || pion::algorithm::iequals(name, "Version")
            || pion::algorithm::iequals(name, "Expires")
            || pion::algorithm::iequals(name, "HttpOnly")
        )
    ));
}
