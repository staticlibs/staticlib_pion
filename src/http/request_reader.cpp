...
const uint32_t request_reader::DEFAULT_READ_TIMEOUT = 10;

tcp::connection_ptr& request_reader::get_connection() {
    return m_tcp_conn;
}

void request_reader::set_timeout(uint32_t seconds) {
    m_read_timeout = seconds;
}

// reader member functions

void request_reader::receive() {
...
void request_reader::consume_bytes(const asio::error_code& read_error, std::size_t bytes_read) {
...
void request_reader::consume_bytes() {
...
void request_reader::read_bytes_with_timeout() {
    if (m_read_timeout > 0) {
...
void request_reader::handle_read_error(const asio::error_code& read_error) {
